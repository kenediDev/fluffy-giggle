...
from database.models.post import Post, Comment
...
class UpdateAPIView(APIView):
    ...
    def post(self, r, pk):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response(
                {"message": _("Post has been updated")}, status=status.HTTP_200_OK
            )
...
class PostModelViewSets(ModelViewSet):
    ...
    def retrieve(self, r, pk):
        ...
        if not queryset:
            return Response(
                {"message": _("Post not found")}, status=status.HTTP_404_NOT_FOUND
            )
        serializer = self.serializer_class(queryset)
        return Response(serializer.data, status=status.HTTP_200_OK)


class CommentsModelViewSets(ModelViewSet):
    ...
    def create(self,r):
        serializer = self.serializer_query(data=r.data)
        serializer.context['args'] = 'comment'
        if serializer.is_valid():
            serializer.save()
    ...
    def destroy(self,r,pk):
        comments = Comment.objects.filter(public_id=pk).first()
        if not comments:
            return Response({'message': _("Comments not found")},status=status.HTTP_404_NOT_FOUND)
        comments.delete()
